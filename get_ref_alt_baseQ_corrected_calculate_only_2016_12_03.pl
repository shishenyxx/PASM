#!/usr/bin/perl
#developed by LiJR corrected by ZhengXN
#version=0.91 modified by Yangxx
#date=2016-07-14
...
#		($p_hom_ref,$p_het,$p_hom_alt,$p_mosaic)=@{$mh_result};
		print "$chr\t$pos\t$depth\t$ref\t$alt1\t$I\t$D\t$count{A}\t$count{C}\t$count{G}\t$count{T}\t$count{a}\t$count{c}\t$count{g}\t$count{t}\t$mle";
