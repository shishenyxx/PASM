...
#### gvcf_path
Path to the variant quality score recalibrated vcfs from haplotype caller. Note that the input should be in vcf format (it is called gvcf only to be distinguished from the vcf input). This will help the calculation such as "near indel".
